...
def select_action(state, steps_done, policy_net, num_actions, start_eps=INITIAL_EXPLORATION, end_eps=FINAL_EXPLORATION,
                  decay_duration=FINAL_EXPLORATION_FRAME):
...
def train(dqn_model, target_model, env, replay_buffer):
    optimizer = optim.RMSprop(dqn_model.parameters(), lr=LEARNING_RATE, momentum=GRADIENT_MOMENTUM, eps=MIN_SQUARED_GRADIENT)
    ...
    for episode in range(NUM_EPISODES):
        ...
        for t in range(T_MAX):
            ...
            if replay_buffer.can_sample(MINIBATCH_SIZE):
                batch, indices, weights = replay_buffer.sample_batch(MINIBATCH_SIZE)
                state_batch, action_batch, reward_batch, next_state_batch, done_batch = batch
                ...
                loss = optimize_model(optimizer, q_values, target_q_values,
                                      weights)  # Incorporate weights into the loss
        ...
        if episode % 10 == 0:
            eps = FINAL_EXPLORATION + (INITIAL_EXPLORATION - FINAL_EXPLORATION) * math.exp(
                -1. * steps_done / FINAL_EXPLORATION_FRAME)
            logging.info(f"Episode: {episode}, Average Reward: {np.mean(episode_rewards[-10:]):.2f},Max Reward: {max_reward} Average Loss: {np.mean(episode_losses[-10:]):.2f},exploration rate: {eps}")
